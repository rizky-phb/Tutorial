...
@section('main')
  <div class="container">
    <h1 class="text-center">Tugas</h1>
    <form id="filterForm" action="{{ route('tugas.index') }}" method="GET" x-data x-ref="filterForm">
      <div class="row justify-content-center d-flex mt-3">
        <div class="col-auto">
          <div class="input-group">
            <input class="form-control" id="searchBar" name="search" type="search" value="{{ request('search') }}" autocomplete="off" placeholder="Cari tugas disini..." required>
      ...
      <div class="row g-2 justify-content-center d-flex mt-2">
        ...
        @if (request('category') || request('search'))
          <div class="col-auto">
            <a class="btn btn-danger text-decoration-none" href="{{ route('list-tugas.index') }}" onclick="loader()">
              <i class="ti ti-reload"></i> Reset
            </a>
          </div>
        @endif
      </div>
      @if ((request('category') || request('search')) && !$tugas->isEmpty())
        <div class="mt-3 text-center">
          <h5>{{ $tugas->total() }} tugas ditemukan</h5>
        </div>
      @endif
    </form>
    @if ($tugas->isEmpty())
            ...
            @foreach ($tugas as $t)
              ...
              <div class="col">
                <div class="card h-100 shadow-sm" x-data="{ favorite: {{ $favorited ? 'true' : 'false' }}, show: false }">
                  <img class="rounded-top" src="{{ $t->cover }}" alt="{{ $t->title }}" style="object-fit: cover;
                                aspect-ratio: 16 / 9;" x-on:mouseenter="show = true" x-on:mouseleave="show = false">
                  <div class="favorite-button shadow" data-tugas-title="{{ $t->title }}" data-tugas-slug="{{ $t->slug }}" data-bs-toggle="tooltip" data-bs-title="Simpan atau hapus favorite" x-show="show" x-on:click="favorite = !favorite" x-on:mouseenter="show = true" x-on:mouseleave="show = false" x-transition.duration.500ms>
                    <i class="ti text-{{ $favorited ? 'warning' : 'black' }} ti-bookmark{{ $favorited ? '-filled' : '' }} fs-5"></i>
                  </div>
                  <div class="card-body">
                    <a class="text-decoration-none" href="{{ route('list-tugas.index', ['category' => $t->category->slug]) }}" style="font-size: 13px">{{ $t->category->name }}</a>
                    <h4 class="card-title d-flex align-items-center gap-2">
                      {{ $t->title }}
                      @if ($t->quiz && $t->quiz->status == 'Published')
                        <span class="badge bg-info" data-bs-toggle="tooltip" data-bs-title="Ada quiz pada tugas ini" style="font-size: 11px; cursor: help;">Tugas</span>
                      @endif
                    </h4>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-2">
                      <div class="badge" style="font-size: 13px; background-color: rgb(234, 234, 234)">
                        <span class="text-muted">
                          ...
                          <span class="favorite-count">{{ $t->favorite }}</span>
                        </span>
                      </div>
                      <a class="btn btn-sm btn-dark" href="{{ route('list-tugas.show', $t->slug) }}">Belajar Sekarang</a>
                    </div>
                  </div>
                </div>
              </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('.favorite-button').click(function() {
        const tugasTitle = $(this).data('tugas-title');
        const tugaslug = $(this).data('tugas-slug');
